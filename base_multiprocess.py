...
def fuckit(name):
        ...
        initial=time.time()
        naem=name
        links=initiator(name)
        results = ThreadPool(len(links)).imap_unordered(downloader,links)    
        print(f'completed one chapter download in {time.time()-initial}')
